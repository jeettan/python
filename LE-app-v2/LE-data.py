...
def saveBeforeData():
    ...
    with open(resource_path('data/LE-data.json'), 'r') as file:
        data = json.load(file)
    ...
    with open(resource_path('data/LE-data.json'), 'w') as file:
        json.dump(data, file)
...
def exportToCSV():
    ...
    with open(resource_path('LE-data.csv'), 'w', newline='') as file:
    ...
    subprocess.Popen(["open", resource_path("LE-data.csv")])
